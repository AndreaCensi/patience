...
/*	td.name { border-right: solid 2px gray; }*/
	...
	tr.some_to_push .to_push { background-color: #aaf; }
...
""")
	...
	for r in data['resources']:
		...
		classes = ["resource"]
		if status.num_modified: classes.append('some_modified')
		else: classes.append('not_modified')
		
		if status.num_untracked: classes.append('some_untracked')
		else: classes.append('not_untracked')

		if status.to_merge: classes.append('some_to_merge')
		else: classes.append('not_to_merge')

		if status.to_push: classes.append('some_to_push')
		else: classes.append('not_to_push')
