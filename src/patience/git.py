...
class Git(Resource):
    ...
    def check_right_branch(self):
        ...
        if not self.is_right_branch():
            ...
            msg = "Currently on branch %r rather than %r" % (cur_branch, self.branch)
    ...
    def branch_exists_remote(self, branch=None):
        ...
        if branch is None:
            branch = self.branch
        res = self.run0("git show-ref --verify refs/remotes/origin/%s" % branch, raise_on_error=False)
        exists = res.ret == 0
        return exists
    ...
    def branch_exists_local(self, branch=None):
        if branch is None:
            branch = self.branch
        res = self.run0("git show-ref --quiet --verify -- refs/heads/%s" % branch, raise_on_error=False)
        exists = res.ret == 0
        return exists
    ...
    def checkout(self):
        self.run(["git", "clone", self.url, self.destination], cwd=".")  # the other was not created yet
    ...
    def something_to_push_to(self, lbranch, rbranch):
        ...
        output = self.runf(
            "git log origin/{rbranch}..{lbranch} --no-merges --pretty=oneline",
            rbranch=rbranch,
            lbranch=lbranch,
        )
    ...
    def something_to_merge_from(self, lbranch, rbranch):
        ...
        output = self.runf(
            "git log  {lbranch}..origin/{rbranch} --no-merges --pretty=oneline",
            rbranch=rbranch,
            lbranch=lbranch,
        )
        commits = linesplit(output)
    ...
    def simple_push_generic(self, lbranch, rbranch):
        ...
        stdout = self.runf("git rev-list {lbranch}..origin/{rbranch}", lbranch=lbranch, rbranch=rbranch)
        if stdout.strip():
            return False
        else:
            return True
