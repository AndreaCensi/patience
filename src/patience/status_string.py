...
status_fields = (
    "present num_modified num_untracked to_push simple_push "
...
def status2string(r, res):
    ...
    if not res.present:
        flags[flag_modified] = "!"
        remarks.append("Dir does not exists. " ' (Should be fixable by "pat checkout".)')
    elif not res.is_git_repo:
        flags[flag_modified] = "!"
        remarks.append("Dir exists but no git repo found." ' (Fix with "git init".)')
    elif not res.has_remote:
        flags[flag_branch_remote] = "!"
        remarks.append("No remote configured." ' (Fix with "git remote add origin %s".)' % r.url)
    else:
        ...
        if res.track == None:
            if res.remote_branch_exists:
                remarks.append("No upstream configured." ' (Should be fixable by "pat fix-upstream".)')
            else:
        ...
        if res.local_branch_exists == False:
            ...
            remarks.append(
                "Local branch %r does not exist." % res.branch + ' (Should be fixable by "pat make-branch".)'
            )
        ...
        if res.remote_branch_exists == False:
            ...
            remarks.append(
                "Remote branch %r does not exist." % res.branch + ' (Should be fixable by "pat push".)'
            )
        ...
        if res.branch_mismatch == True:
            ...
            remarks.append(
                "On branch %r rather than %r." % (res.current_branch, res.branch)
